...
//                saveConfig(bufferedWriter);
    ...
    private void saveConfig(FileOutputStream outputStream){
        if (null == outputStream) {
            return;
        }
        ...
        try {
            ...
            if(line_separator == null || line_separator.trim().equals(""))line_spe = true;
            for (Section section : sections.values()) {
                String strSection = "["+section.getName()+"]\r\n";
                outputStream.write(strSection.getBytes());

                for (Map.Entry<String, Object> entry : section.getValues().entrySet()) {
                    final String key = entry.getKey();
                    final String value = entry.getValue().toString();
                    if (null != key && null != value) {
                        String strValue = entry.getKey() + "=" + entry.getValue().toString() + "\r\n";
