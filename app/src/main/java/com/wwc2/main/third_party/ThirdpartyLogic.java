package com.wwc2.main.third_party;
...
public class ThirdpartyLogic extends BaseLogic {
    ...
    @Override
    public int source() {
        return Define.Source.SOURCE_THIRDPARTY;
    }
    ...
    public boolean onKeyEvent(int keyOrigin, int key, Packet packet) {
        ...
        if (SourceManager.getCurSource() == Define.Source.SOURCE_ACCOFF) {
            ...
            return ret;
            /*-begin-20180424-ydinggen-modify-在后台播音乐，切到其他第三方界面，语音下一首无作用。应该根据后台源判断-*/
        } else if (SourceManager.getCurBackSource() != Define.Source.SOURCE_SILENT) {
            LogUtils.d("media logic onKeyEvent diabale for not SOURCE_SILENT mode.");
            return false;
            /*-end-20180424-ydinggen-modify-在后台播音乐，切到其他第三方界面，语音下一首无作用。应该根据后台源判断-*/
        }
        switch (key) {
